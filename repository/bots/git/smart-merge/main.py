...
if __name__ == '__main__':
    working_dir = (Path.home() / "workflows" / "smart-merge-workspace").resolve()
    projects = {
    ...
    for folder, data in projects.items():
        ...
        base_dir = (working_dir / f"__{folder}")
        SetupThreeWayMerge().execute(setup_3_way_merge.InputDataModel(output_dir=base_dir, repo_infos=repos))
    ...
    unique_words = set()
    for folder, data in projects.items():
        project_directory = (working_dir / f"__{folder}" / "merge_dir").resolve()
    ...
    mapping_file = (working_dir / "rename_mapping.csv")
